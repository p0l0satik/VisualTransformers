...
class CustomMultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, qkv):
        ...
        for i in range(self.num_heads):
            ...
            sf = F.softmax(torch.matmul(Q, torch.transpose(K, -2, -1))/ self.norm_coeff, dim=2)

            attention = (sf ) 
            attention = torch.matmul(attention, V)
            processed_heads.append(self.attention_dropout(attention)) 

        result = self.out_proj(torch.cat(processed_heads, dim=2))
        return result
